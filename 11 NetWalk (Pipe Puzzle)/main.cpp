...
int main()
{
    ...
    sPipe.setOrigin(Vector2f(27,27));
    sComp.setOrigin(Vector2f(18,18));
    sServer.setOrigin(Vector2f(20,20));
         ...
         for(int j=0;j<N;j++)
           {
            ...
            sPipe.setRotation(degrees(p.angle));
            sPipe.setPosition(Vector2f(j*ts,i*ts));sPipe.move(offset);
            app.draw(sPipe);

            if (kind==1)
               { if (p.on) sComp.setTextureRect(IntRect({53,0},{36,36}));
                 else sComp.setTextureRect(IntRect({0,0},{36,36}));
                 sComp.setPosition(Vector2f(j*ts,i*ts));sComp.move(offset);
                 app.draw(sComp);
               }
           }
