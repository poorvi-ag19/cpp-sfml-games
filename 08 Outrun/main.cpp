...
struct Line
{
  ...
  void drawSprite(RenderWindow &app)
  {
    ...
    s.setScale(Vector2f(destW/w,destH/h));
    s.setPosition(Vector2f(destX, destY));
    app.draw(s);
    }
};
...
int main()
{
    RenderWindow app(VideoMode(Vector2u(width, height)), "Outrun Racing!");
    app.setFramerateLimit(60);

    Texture t[50];
    std::vector<std::optional<Sprite>> object(8);
    for(int i=1;i<=7;i++)
     {
    ...
    sBackground.setPosition(Vector2f(-2000,0));
  ...
  if (speed>0) sBackground.move(Vector2f(-lines[startPos].curve*2,0));
  if (speed<0) sBackground.move(Vector2f( lines[startPos].curve*2,0));
